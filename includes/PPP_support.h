...
/*
        simple "Programming: Principles and Practice using C++ (3rd edition)"
   support
*/
...
class Checked_vector : public std::vector<T> { // trivially range-checked vector
                                               // (no iterator checking)
  public:
...
PPP_EXPORT class Checked_string
    : public std::string { // trivially range-checked string (no iterator
                           // checking)
  public:
...
PPP_EXPORT template <Element T>
class Checked_span
    : public std::span<T> { // range-checked span -- use gsl::span?
  public:
...
simple_error(string s) // write ``error: s'' and exit program (for non-exception
...
PPP_EXPORT inline void
error(const std::string &s) // error() simply disguises throws
{
...
PPP_EXPORT template <class T, class U> constexpr T narrow_cast(U &&u) noexcept {
...
PPP_EXPORT template <class T, class U> constexpr T narrow(U u) {
